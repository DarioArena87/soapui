...
public class ShowOnlineHelpAction extends AbstractAction implements HelpActionMarker {
    ...
    public ShowOnlineHelpAction(String title, String url, KeyStroke accelerator, String description, String iconPath) {
        super(title);
        this.url = url;
        putValue(SHORT_DESCRIPTION, description == null ? "Show online help" : description);
        if (accelerator != null) {
            putValue(ACCELERATOR_KEY, accelerator);
        }

        putValue(SMALL_ICON, iconPath == null ? UISupport.HELP_ICON : UISupport.createImageIcon(iconPath));
    }
