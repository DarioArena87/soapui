...
public class WsdlMockResponseTestStep extends WsdlTestStepWithProperties implements OperationTestStep, PropertyChangeListener, Assertable, PropertyExpansionContainer {
    ...
    public void resolve(ResolveContext<?> context) {
        ...
        if (mockOperation == null) {
            ...
            context.addPathToResolve(this, "Missing Operation in Project", mockResponseStepConfig.getInterface() + "/" + mockResponseStepConfig.getOperation())
            ...
            if (context.hasThisModelItem(this, "Missing Operation in Project", mockResponseStepConfig.getInterface() + "/" + mockResponseStepConfig.getOperation())) {
                ...
                PathToResolve path = context.getPath(this,
                                                     "Missing Operation in Project",
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (evt.getSource() == mockResponse || evt.getSource() == mockResponse.getWsaConfig()) {
            if (!evt.getPropertyName().equals(ICON_PROPERTY)) {
                mockResponse.beforeSave();
                mockResponseConfig.set(mockResponse.getConfig());
            }
        ...
        else if (evt.getSource() == getTestCase() &&
                 evt.getPropertyName().equals("testSteps") &&
                 evt.getNewValue() == null &&
                 evt.getOldValue() == startTestStep &&
                 startTestStep != null) {
            setStartStep(null);
        }
        else if (evt.getSource() == startTestStep && evt.getPropertyName().equals(NAME_PROPERTY)) {
    ...
    public class InternalMockRunListener extends MockRunListenerAdapter {
        ...
        public synchronized void onMockResult(MockResult result) {
            ...
            // is this for us?
            if (lastResult == null && waiting && result.getMockResponse() == testMockResponse) {
                waiting = false;
                System.out.println("Got mockrequest to [" + getName() + "]");
                // save
                lastResult = (WsdlMockResult)result;
    ...
    private class PropertyChangeNotifier {
        private final AssertionStatus oldStatus;
        private final ImageIcon oldIcon;
    ...
    private class AssertedWsdlMockResultMessageExchange extends WsdlMockResultMessageExchange implements RequestAssertedMessageExchange, AssertedXPathsContainer {
