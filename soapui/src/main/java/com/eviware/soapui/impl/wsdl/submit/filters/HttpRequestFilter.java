...
public class HttpRequestFilter extends AbstractRequestFilter {
    ...
    public void filterHttpRequest(SubmitContext context, HttpRequestInterface<?> request) {
        ...
        MimeMultipart formMp = ("multipart/form-data".equals(request.getMediaType()) || "multipart/mixed".equals(request.getMediaType())) &&
        ...
        for (int c = 0; c < params.getPropertyCount(); c++) {
            ...
            List<String> valueParts = sendEmptyParameters(request) || (!StringUtils.hasContent(value) && param.getRequired())
                                      ? RestUtils.splitMultipleParametersEmptyIncluded(value, request.getMultiValueDelimiter())
        ...
        if (PathUtils.isHttpPath(path)) {
            try {
                ...
                httpMethod.setURI(HttpUtils.createUri(oldUri.getScheme(),
