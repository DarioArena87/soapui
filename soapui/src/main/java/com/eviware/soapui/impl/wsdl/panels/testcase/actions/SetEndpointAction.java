...
public class SetEndpointAction extends AbstractAction {
    ...
    public SetEndpointAction(WsdlTestCase testCase) {
        ...
        putValue(SMALL_ICON, UISupport.createImageIcon("/set_endpoint.gif"));
        putValue(SHORT_DESCRIPTION, "Sets the endpoint for all requests in this testcase");
    }
        ...
        for (int c = 0; c < testCase.getTestStepCount(); c++) {
            ...
            if (step instanceof HttpRequestTestStep) {
                ...
                if (operation != null) {
                    ...
                    Collections.addAll(endpointSet, endpoints);
                }
                currentEndpointSet.add(requestStep.getTestRequest().getEndpoint());
            }
        }

        String selected = (String)UISupport.prompt("Select endpoint to set for all requests",
                                                   "Set Endpoint",
                                                   endpointSet.toArray(),
