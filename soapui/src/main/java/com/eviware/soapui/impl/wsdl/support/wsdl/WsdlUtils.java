...
public class WsdlUtils {
    ...
    public static String getSoapAction(BindingOperation operation) {
        ...
        SOAPOperation soapOperation = getExtensiblityElement(list, SOAPOperation.class);
        if (soapOperation != null) {
            return soapOperation.getSoapActionURI();
        }

        SOAP12Operation soap12Operation = getExtensiblityElement(list, SOAP12Operation.class);
    ...
    public static boolean isInputSoapEncoded(BindingOperation bindingOperation) {
        ...
        SOAPBody soapBody = getExtensiblityElement(bindingInput.getExtensibilityElements(), SOAPBody.class);

        if (soapBody != null) {
            return soapBody.getUse() != null &&
                   soapBody.getUse().equalsIgnoreCase("encoded") &&
                   (soapBody.getEncodingStyles() == null || soapBody.getEncodingStyles().contains("http://schemas.xmlsoap.org/soap/encoding/"));
        }

        SOAP12Body soap12Body = getExtensiblityElement(bindingInput.getExtensibilityElements(), SOAP12Body.class);

        if (soap12Body != null) {
            return soap12Body.getUse() != null &&
                   soap12Body.getUse().equalsIgnoreCase("encoded") &&
                   (soap12Body.getEncodingStyle() == null || soap12Body.getEncodingStyle().equals("http://schemas.xmlsoap.org/soap/encoding/"));
        }
    ...
    public static boolean isOutputSoapEncoded(BindingOperation bindingOperation) {
        ...
        SOAPBody soapBody = getExtensiblityElement(bindingOutput.getExtensibilityElements(), SOAPBody.class);
        ...
        SOAP12Body soap12Body = getExtensiblityElement(bindingOutput.getExtensibilityElements(), SOAP12Body.class);
    ...
    public static boolean isRpc(Definition definition, BindingOperation bindingOperation) {
        SOAPOperation soapOperation = getExtensiblityElement(bindingOperation.getExtensibilityElements(), SOAPOperation.class);

        if (soapOperation != null && soapOperation.getStyle() != null) {
            return soapOperation.getStyle().equalsIgnoreCase("rpc");
        }

        SOAP12Operation soap12Operation = getExtensiblityElement(bindingOperation.getExtensibilityElements(), SOAP12Operation.class);
    ...
    public static Part[] getInputParts(BindingOperation operation) {
        ...
        if (msg != null) {
            SOAPBody soapBody = getExtensiblityElement(operation.getBindingInput().getExtensibilityElements(), SOAPBody.class);

            if (soapBody == null || soapBody.getParts() == null) {
                SOAP12Body soap12Body = getExtensiblityElement(operation.getBindingInput().getExtensibilityElements(), SOAP12Body.class);
    ...
    public static MIMEContent[] getOutputMultipartContent(Part part, BindingOperation operation) {
        ...
        MIMEMultipartRelated multipartOutput = getExtensiblityElement(output.getExtensibilityElements(), MIMEMultipartRelated.class);
    ...
    public static MIMEContent[] getInputMultipartContent(Part part, BindingOperation operation) {
        ...
        MIMEMultipartRelated multipartInput = getExtensiblityElement(bindingInput.getExtensibilityElements(), MIMEMultipartRelated.class);
    ...
    public static MIMEContent[] getContentParts(Part part, MIMEMultipartRelated multipart) {
        ...
        if (multipart != null) {
            ...
            for (int c = 0; c < parts.size(); c++) {
                List<MIMEContent> contentParts = getExtensiblityElements(parts.get(c).getExtensibilityElements(), MIMEContent.class);
    ...
    public static String findSoapFaultPartName(
        SoapVersion soapVersion, BindingOperation bindingOperation, String message
    ) throws Exception {
    ...
    public static String getUsingAddressing(ElementExtensible item) {
        ...
        Element[] usingAddressingElements = getExentsibilityElements(item, new QName("http://www.w3.org/2006/05/addressing/wsdl", "UsingAddressing"));
        if (usingAddressingElements.length == 0) {
            usingAddressingElements = getExentsibilityElements(item, new QName("http://www.w3.org/2006/02/addressing/wsdl", "UsingAddressing"));
        }
        ...
        if (usingAddressingElements.length != 0) {
            ...
            if ("true".equals(addressingVersion)) {
    ...
    public static String getSoapBodyNamespace(List<?> list) {
        SOAPBody soapBody = getExtensiblityElement(list, SOAPBody.class);
        if (soapBody != null) {
            return soapBody.getNamespaceURI();
        }

        SOAP12Body soap12Body = getExtensiblityElement(list, SOAP12Body.class);
        if (soap12Body != null) {
    ...
    public static String replacePortEndpoint(WsdlInterface iface, InputSource inputSource, String endpoint) throws WSDLException {
        ...
        if (updated) {
            ...
            if (!nsMap.containsValue(Constants.SOAP_HTTP_BINDING_NS)) {
    ...
    public static BindingOperation findBindingOperation(
        Binding binding, String bindingOperationName, String inputName, String outputName
    ) {
        if (binding == null) {
            return null;
        }
    ...
    public static boolean isHeaderInputPart(Part part, Message message, BindingOperation bindingOperation) {
        List<SOAPHeader> headers = getExtensiblityElements(bindingOperation.getBindingInput().getExtensibilityElements(), SOAPHeader.class);
    ...
    public static boolean isHeaderOutputPart(Part part, Message message, BindingOperation bindingOperation) {
        BindingOutput bindingOutput = bindingOperation.getBindingOutput();
        List<SOAPHeader> headers = bindingOutput == null ? null : getExtensiblityElements(bindingOutput.getExtensibilityElements(), SOAPHeader.class);
    ...
    public static void getAnonymous(WsdlOperation wsdlOperation) {
        ...
        Element[] anonymousElements = getExentsibilityElements(wsdlOperation.getBindingOperation(), new QName("http://www.w3.org/2006/05/addressing/wsdl", "Anonymous"));
        if (anonymousElements.length == 0) {
            anonymousElements = getExentsibilityElements(wsdlOperation.getBindingOperation(), new QName("http://www.w3.org/2006/02/addressing/wsdl", "Anonymous"));
        }
    ...
    public static String getDefaultWsaAction(WsdlOperation operation, boolean output) {
        ...
        try {
            ...
            for (String namespace : WsaUtils.wsaNamespaces) {
                attrs = getExentsibilityAttributes(attributeExtensible, new QName(namespace, "Action"));
            ...
            String targetNamespace = getTargetNamespace(definition);
            ...
            if (!StringUtils.isNullOrEmpty(operationName)) {
                ...
                if (op != null) {
                    ...
                    attrs = getExentsibilityAttributes(attributeExtensible, new QName(WsaUtils.WS_A_NAMESPACE_200705, "Action"));
                    if (attrs != null && attrs.length > 0) {
                        return attrs[0];
                    }
                }
