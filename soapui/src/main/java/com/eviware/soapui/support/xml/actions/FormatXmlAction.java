...
public class FormatXmlAction extends AbstractAction {
    ...
    public FormatXmlAction(RSyntaxTextArea editArea, String title) {
        ...
        putValue(SHORT_DESCRIPTION, "Pretty-prints the xml");
        if (UISupport.isMac()) {
            putValue(ACCELERATOR_KEY, UISupport.getKeyStroke("shift meta F"));
        }
        else {
            putValue(ACCELERATOR_KEY, UISupport.getKeyStroke("alt F"));
        }
        textArea = editArea;
    }
