...
public class WsdlRequestWsrmInspector extends AbstractWsrmInspector implements XmlInspector, SubmitListener {
    ...
    public void buildContent(SimpleBindingForm form) {
        ...
        form.appendComboBox("version", "WS-RM Version", new String[]{
    ...
    public boolean beforeSubmit(Submit submit, SubmitContext context) {
        ...
        if (container.getWsrmConfig().isWsrmEnabled()) {
            ...
            WsrmSequence sequence = utils.createSequence(request.getEndpoint(),
                                                         iface.getSoapVersion(),
                                                         request.getWsrmConfig().getVersionNameSpace(),
                                                         request.getWsrmConfig().getAckTo(), 0L,
                                                         request.getOperation(),
                                                         ((WsdlRequest)submit.getRequest()).getWsaConfig().getTo(),
                                                         request.getWsrmConfig().getOfferEndpoint()
            );
    ...
    public void afterSubmit(Submit submit, SubmitContext context) {
        ...
        if (container.getWsrmConfig().isWsrmEnabled()) {
            ...
            utils.closeSequence(
                ...
                iface.getSoapVersion(),
                request.getWsrmConfig().getVersionNameSpace(),
                request.getWsrmConfig().getUuid(),
                request.getWsrmConfig().getSequenceIdentifier(), 1L,
                request.getOperation()
            );
        }
    }
