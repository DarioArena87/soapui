...
public class ProgressBarSecurityScanAdapter extends SecurityTestRunListenerAdapter {
    ...
    private final String prePostFix = " ";

    private final JLabel cntLabel;
    private final Color defaultBackground;

    public ProgressBarSecurityScanAdapter(
        JTree tree, SecurityScanNode node, JProgressBar progressBar, SecurityScan securityCheck, SecurityTest securityTest, JLabel cntLabel
    ) {
        this.tree = tree;
        this.node = node;
        this.progressBar = progressBar;
        defaultBackground = progressBar.getBackground();
    ...
    public void afterSecurityScan(
        ...
        if (securityCheckResult.getSecurityScan().getTestStep().getId().equals(securityCheck.getTestStep().getId()) &&
            securityCheck.getName().equals(securityCheckResult.getSecurityScan().getName())) {
            if (securityCheckResult.getStatus() != ResultStatus.CANCELED) {
                ...
                if (securityCheck.getAssertionCount() == 0) {
                    ...
                    if (securityCheck instanceof AbstractSecurityScanWithProperties) {
                        AbstractSecurityScanWithProperties scanWithProperties = (AbstractSecurityScanWithProperties)securityCheck;
                        if (scanWithProperties.getParameterHolder() != null &&
                            (scanWithProperties.getParameterHolder().getParameterList() != null && (scanWithProperties.getParameterHolder().getParameterList().size() == 0))) {
                            progressBar.setForeground(MISSING_ASSERTION_COLOR);
    ...
    public void afterSecurityScanRequest(
        ...
        if (securityCheckReqResult.getSecurityScan().getTestStep().getId().equals(securityCheck.getTestStep().getId()) &&
            securityCheck.getName().equals(securityCheckReqResult.getSecurityScan().getName())) {
            if (securityCheck.getAssertionCount() == 0) {
                progressBar.setForeground(MISSING_ASSERTION_COLOR);
                progressBar.setString(STATE_MISSING_ASSERTIONS);
            ...
            else if (securityCheck instanceof AbstractSecurityScanWithProperties) {
                if (((AbstractSecurityScanWithProperties)securityCheck).getParameterHolder() != null &&
                    ((AbstractSecurityScanWithProperties)securityCheck).getParameterHolder().getParameterList() != null &&
