...
public abstract class AbstractBoundary implements Boundary {
    ...
    public static String outOfBoundaryValue(String baseType, String nodeName, String nodeValue) {
        Boundary boundary = factory(baseType);

        if (XSD_MIN_LENGTH.equals(nodeName)) {
            return boundary.outOfBoundary(MIN_LENGTH, nodeValue);
        }
        else if (XSD_MAX_LENGTH.equals(nodeName)) {
            return boundary.outOfBoundary(MAX_LENGTH, nodeValue);
        }
        else if (XSD_LENGTH.equals(nodeName)) {
            return boundary.outOfBoundary(LENGTH, nodeValue);
        }
        else if (XSD_MAX_EXCLUSIVE.equals(nodeName)) {
            return boundary.outOfBoundary(MAX_EXCLISIVE, nodeValue);
        }
        else if (XSD_MAX_INCLUSIVE.equals(nodeName)) {
            return boundary.outOfBoundary(MAX_INCLISIVE, nodeValue);
        }
        else if (XSD_MIN_EXCLUSIVE.equals(nodeName)) {
            return boundary.outOfBoundary(MIN_EXCLISIVE, nodeValue);
        }
        else if (XSD_MIN_INCLUSIVE.equals(nodeName)) {
            return boundary.outOfBoundary(MIN_INCLISIVE, nodeValue);
        }
        else if (XSD_TOTAL_DIGITS.equals(nodeName)) {
            return boundary.outOfBoundary(TOTAL_DIGITS, nodeValue);
        }
        else if (XSD_FRACTION_DIGITS.equals(nodeName)) {
            return boundary.outOfBoundary(FRACTION_DIGITS, nodeValue);
        }
        return null;
    }
}
