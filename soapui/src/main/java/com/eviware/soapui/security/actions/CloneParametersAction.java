...
public class CloneParametersAction extends AbstractAction {
    ...
    public CloneParametersAction() {
        ...
        putValue(SMALL_ICON, UISupport.createImageIcon("/clone_parameters.png"));
        putValue(SHORT_DESCRIPTION, "Clones parameter");
        setEnabled(false);
    }
    ...
    public List<ModelItem> performClone(boolean showErrorMessage) {
        ...
        for (String scanName : targetSecurityScans) {
            AbstractSecurityScanWithProperties targetSecurityScan = (AbstractSecurityScanWithProperties)targetSecurityTest.getTestStepSecurityScanByName(
                targetTestStep.getId(),
                scanName
            );
    ...
    protected XFormDialog createCloneParameterDialog() {
        ...
        TestCase testCase = securityScan.getTestStep().getTestCase();
    ...
    @AForm(
        description = "Specify target TestSuite/TestCase/Security Test(s)/Security Scan(s) and select Parameters to clone",
        name = "Clone Parameters",
        icon = UISupport.TOOL_ICON_PATH,
        helpUrl = HelpUrls.SECURITY_SCANS_OVERVIEW
    )
